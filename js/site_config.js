...
TerminalShell.commands['xkcd-next'] = function(terminal) {
	xkcdDisplay(terminal, xkcd.last.num+1);
};

TerminalShell.commands['xkcd-previous'] =
TerminalShell.commands['xkcd-prev'] = function(terminal) {
	xkcdDisplay(terminal, xkcd.last.num-1);
};

TerminalShell.commands['xkcd-first'] = function(terminal) {
	xkcdDisplay(terminal, 1);
};

TerminalShell.commands['xkcd-latest'] =
TerminalShell.commands['xkcd-last'] = function(terminal) {
	xkcdDisplay(terminal, xkcd.latest.num);
};

TerminalShell.commands['xkcd-random'] = function(terminal) {
	xkcdDisplay(terminal, getRandomInt(1, xkcd.latest.num));
};

TerminalShell.commands['xkcd-goto'] = function(terminal, subcmd) {
	$('#screen').one('cli-ready', function(e) {
		terminal.print('Did you mean "xkcd-display"?');
	});
...
TerminalShell.commands['poweroff'] = function(terminal) {
	if (this.sudo) {
		terminal.print('Broadcast message from zero@iternull');
		terminal.print();
...
Filesystem = {
	'welcome.txt': {type:'file', read:function(terminal) {
		...
		terminal.print("");
		terminal.print($('<p>').html('Links: <a target="_blank" href="https://www.iternull.com/about/">About</a> <a target="_blank" href="https://blog.iternull.com/">Blog</a> <a target="_blank" href="https://www.iternull.com/projects/">Projects</a> <a target="_blank" href="https://github.com/iternull">GitHub</a> '));
		terminal.print("");
	}}/* ,
...
// Filesystem['shop'] = linkFile('https://shop.iternull.com/');
// Filesystem['store'] = linkFile('https://store.iternull.com/');
// Filesystem['forums'] = linkFile('https://forums.iternull.com/');
...
TerminalShell.commands['echo'] = function(terminal, text) {
    terminal.print(text);
};
...
TerminalShell.commands['cheat'] = function(terminal) {
	terminal.print($('<a>').text('*** FREE SHIPPING ENABLED ***').attr('href', 'http://store.iternull.com/'));
};
...
//	terminal.print('That would be cheating!');
	...
	terminal.print('');
	terminal.print('  ls           - List information about the FILEs.');
	terminal.print('  cd           - Change the working directory.');
	terminal.print('  rm           - Remove (unlink) the FILE(s).');
	terminal.print('  cat          - Print on the standard output.');
	terminal.print('  echo         - Display a line of text.');
	terminal.print('  sleep        - Pause for NUMBER seconds.');
    terminal.print('  clear        - Clear the terminal screen.');
	terminal.print('  sudo         - Execute a command as another user.');
	terminal.print('  quit         - Quit terminal');
...
*/
	...
	$(document).konami(function(){
		...
		if (konamiCount == 0) {
			...
			$('#screen').css('background', 'url("") center no-repeat');
		}
